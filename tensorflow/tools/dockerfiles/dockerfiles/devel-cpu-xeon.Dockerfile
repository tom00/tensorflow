...
RUN git checkout v2.1.0-rc1

FROM build1 as build2

RUN yes '' | ./configure
RUN bazel build --config=mkl --config=v2 --copt=-mavx --copt=-mavx2 --copt=-mfma --copt=-msse4.2 --config=opt //tensorflow/tools/pip_package:build_pip_package 
